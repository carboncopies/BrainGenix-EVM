 ...
 *        to the vector index neuron in A. Negative index means no match, which can
 *        occur if there are more points in A than in B.
 * @param _TryAngles Number of angles to try on each axis.
 * @return True if successfully registered.
 */
bool SimpleRegistration(SafeClient & _Client, int _SimIDA, int _SimIDB, std::vector<int> & _RegistrationMap, unsigned int _TryAngles) {
	...
	std::vector<size_t> FromANearestInB;
	Vec3D BestRotationAngles = BruteForceBestMatchAngles(SomaCentersA, SomaCentersB, &FromANearestInB, _TryAngles);
	_RegistrationMap.resize(FromANearestInB.size());
	for (size_t i = 0; i < FromANearestInB.size(); i++) {
		_RegistrationMap[i] = FromANearestInB[i]; // from size_t to int
	}
	if (SomaCentersA.size() > SomaCentersB.size()) {
		MakeRegistrationMapUnique(SomaCentersA, SomaCentersB, BestRotationAngles, _RegistrationMap);
	}
